...
const TopBrandsPage = () => {
  ...
  useEffect(() => {
    const dummyProducts = Array.from({ length: 30 }, (_, i) => ({
      ...
      image: `https://picsum.photos/seed/brand${i + 1}/300/350`,
    }));
  ...
  const filteredProducts =
    selectedFilters.brand.length === 0
      ? products
      : products.filter((p) => selectedFilters.brand.includes(p.brand));

  return (
    <div className="min-h-screen bg-yellow-50 pt-24 pb-10 px-6">
      <h1 className="text-4xl font-extrabold text-center mb-10 text-yellow-700">
        🔥 Top Brands — Shop the Best Labels
      </h1>
